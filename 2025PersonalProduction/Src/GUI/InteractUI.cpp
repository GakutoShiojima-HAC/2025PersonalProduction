...
void InteractUI::draw(const std::vector<Actor*>& actors, int target_index) {
    ...
    for (size_t i = 0; i < INTERACT_LIST; ++i) {
        ...
        // テキストを取得
        std::string text = MyString::utf8_to_shiftjis((index == target_index ? ">> " : "")) + actor->interact_text() + MyString::utf8_to_shiftjis((index == target_index ? " <<" : ""));
        // テキストを描画
        Canvas::draw_sprite_text(
            text,                                           // テキスト
            GSvector2{ 1200.0f, 650.0f + (20.0f * i) },     // 描画位置
            40.0f,                                          // フォントサイズ
            cFONT,
            GS_FONT_NORMAL,
